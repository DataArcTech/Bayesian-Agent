...
两个 backend 都会进入同一套 Skill 排序、posterior 加权 context 渲染，以及 `patch`、`split`、`compress`、`retire`、`explore` 等 rewrite actions。完整的多 Skill hypothesis Bayesian model selection 在 roadmap 中，不作为 v0.x 已完成能力来宣传。
